import { buildCurrentLocationReviewQueue } from '../twitch-stream-map-current-location/reviewability.mjs'

const HEALTH_PATH = '/health'
const PROBE_PATH = '/audit/current-review-queue'
const PAGE_SIZE = 100
const MAX_PAGES = 3
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === HEALTH_PATH) {
      ...
      return json({
        ok: true,
        mode: 'current_location_review_queue_top300_preview',
        ...
        identityPreservingCandidateArtifact: true,
        credentialPresence: {
    ...
    try {
      return json({ ok: true, result: await runCurrentLocationReviewQueueProbe({ env }) })
    } catch (error) {
...
export async function runCurrentLocationReviewQueueProbe({ env, fetchImpl = fetch, now = () => new Date() }) {
  ...
  if (!inMemoryRows.length) throw new Error('empty_current_review_sample')
  const observedAt = now().toISOString()
  const review = buildCurrentLocationReviewQueue(inMemoryRows, { observedAt })

  return {
    schemaVersion: 'viewloom-twitch-stream-map-current-review-queue-live-probe-v0.1',
    provider: 'twitch',
    mode: 'current_location_review_queue_top300_preview',
    observedAt,
    ...
    fieldsInspectedInMemory: ['user_id', 'user_login', 'title', 'tags', 'language'],
    fieldsReturned: ['stable_identity', 'login', 'public_channel_reference', 'candidate_places', 'candidate_source_classes', 'review_state', 'review_window'],
    review,
  }
}
