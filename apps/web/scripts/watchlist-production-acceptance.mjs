...
try {
  ...
  try {
    evidence.scenarios.push(await verifyHomeEntry(browser, twitch, {
      width: 1280,
      height: 900,
      mobile: false,
      screenshot: '/tmp/watchlist-w5b-twitch-home.png',
    }))
    evidence.scenarios.push(await verifyHomeEntry(browser, kick, {
      width: 390,
      height: 844,
      mobile: true,
    ...
    evidence.scenarios.push(await verifyChannelSave(browser, twitch, '/tmp/watchlist-w5b-twitch-channel-save.png'))
    evidence.scenarios.push(await verifyChannelSave(browser, kick, '/tmp/watchlist-w5b-kick-channel-save.png'))
  } finally {
    await browser.close()
  }
  ...
  await writeEvidence()
  log('Watchlist W5B production acceptance passed.')
} catch (error) {
...
async function inspectProvider(provider) {
  ...
  check(status?.collector?.state === 'ok', `${provider} collector is not ok.`)
  ...
  check(ids.length >= 3, `${provider} did not provide enough distinct production states.`)
  ...
  return {
    ...
    summary: {
      ...
      collectorState: status.collector.state,
...
async function verifyHomeEntry(browser, data, viewport) {
  const { provider } = data
  ...
  await page.locator('.provider-utility__item').waitFor()

  check(await page.title() === `${providerName} Data — ViewLoom`, `${provider} Home title mismatch.`)
  ...
  check((await utility.innerText()).includes('Local Watchlist'), `${provider} Home is missing the Local Watchlist utility label.`)
  check(await utility.getAttribute('href') === `/${provider}/watchlist/`, `${provider} Home Watchlist href mismatch.`)
  check(await page.locator('.feature-directory a[href*="watchlist"]').count() === 0, `${provider} Home inserted Watchlist into the primary feature directory.`)

  await assertNoOverflow(page, `${provider} Home ${viewport.width}px production`)
...
async function verifyWatchlist(browser, data, viewport) {
  ...
  const expectedKey = `viewloom.watchlist.${provider}.v1`
  ...
  const firstResponse = await page.goto(`${origin}${route}?watchlist-w5b=${Date.now()}`, {
    waitUntil: 'domcontentloaded',
    timeout: 30_000,
  })
  check(firstResponse?.status() === 200, `${provider} Watchlist did not return 200.`)
  await ready(page)
  assertCounts(calls, paths, { latest: 0, history: 0 }, `${provider} empty initial load`)
  check(await page.locator('[data-watchlist-empty]').isVisible(), `${provider} production empty state is missing.`)
  check(await page.title() === `${providerName} Local Watchlist — ViewLoom`, `${provider} Watchlist title mismatch.`)
  ...
  check(await page.locator('[data-watchlist-entry="cross_provider_probe"]').count() === 0, `${provider} rendered the other provider storage entry.`)

  const storageSnapshot = await page.evaluate(({ expectedKey, otherKey }) => ({
    current: JSON.parse(localStorage.getItem(expectedKey) || 'null'),
    other: JSON.parse(localStorage.getItem(otherKey) || 'null'),
  }), { expectedKey, otherKey })
  check(storageSnapshot.current?.provider === provider, `${provider} current storage provider mismatch.`)
  check(storageSnapshot.current?.entries?.length === data.entries.length, `${provider} current storage entry count mismatch.`)
  check(storageSnapshot.other?.provider === otherProvider, `${provider} other provider storage was not preserved independently.`)
  check(storageSnapshot.other?.entries?.[0]?.channelId === 'cross_provider_probe', `${provider} other provider storage changed unexpectedly.`)
  ...
  const external = latestCard.locator('.watchlist-external')
  const externalHref = await external.getAttribute('href')
  check(
    provider === 'twitch'
      ? externalHref?.startsWith('https://www.twitch.tv/')
      : externalHref?.startsWith('https://kick.com/'),
    `${provider} external channel link crossed providers.`,
  )
  ...
  assertDelta(calls, paths, before, { latest: 0, history: 0 }, `${provider} local add`)
  ...
  assertDelta(calls, paths, before, { latest: 0, history: 0 }, `${provider} local remove`)
  ...
  await assertNoOverflow(page, `${provider} ${viewport.width}px production`)
  if (viewport.mobile) {
...
async function verifyChannelSave(browser, data, screenshot) {
  const { provider } = data
  const otherProvider = provider === 'twitch' ? 'kick' : 'twitch'
  const providerKey = `viewloom.watchlist.${provider}.v1`
  const otherKey = `viewloom.watchlist.${otherProvider}.v1`
  ...
  await page.goto(`${origin}/${provider}/watchlist/?watchlist-w5b=${Date.now()}`, {
    waitUntil: 'domcontentloaded',
    timeout: 30_000,
  })
  ...
  const channelResponse = await page.goto(`${origin}/${provider}/channel/?id=${encodeURIComponent(data.latestPresent)}&watchlist-w5b=${Date.now()}`, {
    waitUntil: 'domcontentloaded',
    timeout: 30_000,
  })
  check(channelResponse?.status() === 200, `${provider} Channel did not return 200.`)
  ...
  check(await managementLink.getAttribute('href') === `/${provider}/watchlist/`, `${provider} saved management link mismatch.`)

  const stored = await page.evaluate(({ providerKey, otherKey }) => ({
    current: JSON.parse(localStorage.getItem(providerKey) || 'null'),
    other: JSON.parse(localStorage.getItem(otherKey) || 'null'),
  ...
  check(stored.other?.entries?.[0]?.channelId === 'other_provider_saved', `${provider} Channel save changed the other provider storage.`)
  check(
    (await page.locator('[data-channel-watchlist-feedback]').innerText()).includes('No data request was made.'),
    `${provider} Channel save feedback is incorrect.`,
  )
...
async function appendLog(value) {
  await appendFile('/tmp/watchlist-w5b.log', `${JSON.stringify(value)}\n`)
}

async function writeEvidence() {
  evidence.diagnostics = diagnostics.slice(-250)
  await writeFile('/tmp/watchlist-w5b-evidence.json', `${JSON.stringify(evidence, null, 2)}\n`)
}
