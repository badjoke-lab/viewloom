...
const evidence = {
  schema: 'viewloom-history-ui-h3-overview-v1',
  phase: 'P9H3',
...
async function run(browser, provider, viewport, touch) {
  ...
  if (!touch) {
    ...
    assert.ok(next.openGroups.includes('ranking'))
    assert.ok(next.visibleSecondaryGroups >= 3)
    ...
    assert.ok(next.openGroups.includes('coverage'))
    assert.ok(!next.openGroups.includes('ranking'))
    assert.equal(calls.length, before)
  }

  await page.screenshot({ path: resolve(out, `${provider}-${viewport.width}-final.png`), fullPage: true })
  evidence.scenarios.push({ id: `${provider}-overview-${viewport.width}`, provider, viewport, touch, calls, initial, final: await snapshot(page) })
  await context.close()
}
...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    const mobile = document.querySelector('[data-history-mobile-analysis]')
    return {
      ...
      mobileNavDisplay: mobile instanceof HTMLElement ? getComputedStyle(mobile).display : 'missing',
      visibleSecondaryGroups: groups.filter(isVisible).length,
      openGroups: [...new Set(groups.filter((node) => node.classList.contains('is-mobile-open')).map((node) => node.getAttribute('data-history-secondary-group')))].filter(Boolean),
      ...
      geometry: { overviewWidth: width('[data-history-view-panel="overview"]'), primaryWidth: width('[data-history-columns]'), navigationWidth: width('[data-history-mobile-analysis]') },
      order: { summary: top('[data-history-summary]'), primary: top('[data-history-columns]'), comparison: top('.history-period-comparison-block'), calendar: top('.history-calendar-block'), ranking: top('.history-overview-ranking-title'), insights: top('[data-history-overview-insights]'), coverage: top('.history-overview-coverage-title') },
    }
  })
}
