...
const battleLinkSource = read('src/navigation/battle-lines-deep-link-bridge.ts')
...
assert(battleSource.includes('readBattleLinesSelection(params)'), 'Battle Lines legacy selection reader is not owned by the controller.')
assert(battleLinkSource.includes("params.get('point')"), 'Legacy Battle Lines point read is missing from the pure helper.')
assert(battleSource.includes("next.set('time', time)"), 'Battle Lines canonical selected-time write is missing.')
assert(!battleSource.includes("next.set('point'"), 'Battle Lines controller must not emit legacy point indexes.')
...
const copyEntryPath = 'src/navigation/copy-current-view.ts'
const copyPath = 'src/navigation/copy-current-view-impl.ts'
const copyEntry = read(copyEntryPath)
const copySource = read(copyPath)
const analyticsSource = read('src/analytics.ts')
assert(copyEntry.includes("from './copy-current-view-impl'"), `${copyEntryPath}: implementation entry is missing.`)
