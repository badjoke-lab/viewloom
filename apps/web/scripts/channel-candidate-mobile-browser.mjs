...
async function runMobileDays(browser) {
...
async function runNarrowOverview(browser) {
  const calls = { twitch: 0, kick: 0 }
  ...
  await page.locator('[data-channel-view="overview"]').focus()
  const focus = await page.locator('[data-channel-view="overview"]').evaluate((node) => {
...
try {
  await runMobileDays(browser)
  await runNarrowOverview(browser)
  console.log('Channel C5A mobile visual and accessibility gate passed.')
} finally {
  await browser.close()
}
