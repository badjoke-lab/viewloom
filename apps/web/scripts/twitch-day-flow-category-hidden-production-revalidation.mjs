...
try {
  await scenario(browser, {
    ...
    run: async ({ page, record, requests }) => {
      ...
      assert(json.rangeMode === 'date', `normal Twitch Day Flow rangeMode=${json.rangeMode}`)
      assert(json.selectedDate === VALIDATION_DATE, `normal Twitch Day Flow selectedDate=${json.selectedDate}`)
      ...
      record.checks = { rangeMode: json.rangeMode, selectedDate: json.selectedDate, bucketCount: json.buckets.length, bandCount: json.bands.length, pageGeometry }
  ...
  await scenario(browser, {
    ...
    run: async ({ page, record }) => {
      ...
      const nav = await page.goto(`${ORIGIN}/twitch/day-flow/?categoryPreview=1&category=${encodeURIComponent(unknown)}&rangeMode=date&date=${VALIDATION_DATE}&auto=off`, { waitUntil: 'domcontentloaded', timeout: 45000 })
      ...
      record.checks = { rangeMode: json.rangeMode, selectedDate: json.selectedDate, state: json.categoryFilter?.state, bandCount: json.bands.length, statusText }
    },
  })
  ...
  await scenario(browser, {
    ...
    run: async ({ page, record, requests }) => {
      ...
      assert(json.rangeMode === 'date', `Kick Day Flow rangeMode=${json.rangeMode}`)
      assert(json.selectedDate === VALIDATION_DATE, `Kick Day Flow selectedDate=${json.selectedDate}`)
      ...
      record.checks = { rangeMode: json.rangeMode, selectedDate: json.selectedDate, bucketCount: json.buckets.length, bandCount: json.bands?.length ?? 0, pageGeometry }
