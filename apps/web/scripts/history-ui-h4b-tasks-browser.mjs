...
async function desktopScenario(browser) {
  ...
  assert.deepEqual(state.publishGroups.map((group) => group.buttons), [2, 2, 2], 'desktop: publishing actions are grouped incorrectly')
  assert.match(state.publishGroups[0].text, /Copy text.*Full report or short post/i, 'desktop: Copy group explanation missing')
  assert.match(state.publishGroups[0].text, /Share report|Share short post/i, 'desktop: native share action is not grouped with report text')
