...
type DayAgg = { day: string; totalViewerMinutes: number; peakViewers: number; peakStreamerName: string | null; peakStreamerViewers: number; observedStreamCount: number; observedMinutes: number; coverageState: string; demoCount: number }
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (requestedDays > 90) return errorResponse(period, metric, 'range_too_large', 'History custom range is limited to 90 days in v1.', 400)
  ...
  try {
    ...
    return Response.json({
      ...
      coverage,
      notes: [
        'Kick History uses observed minute snapshots and payload-level streamer aggregation.',
        'Twitch and Kick history are intentionally not mixed in v1.',
      ],
    }, { headers: { 'cache-control': 'no-store' } })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Kick History API failed.'
...
async function fetchRows(env: Env, from: string, to: string): Promise<SnapshotRow[]> {
  ...
  `).bind('kick', `${from}T00:00:00.000Z`, nextDayIso(to)).all<SnapshotRow>()
...
function buildHistory(rows: SnapshotRow[]): { days: Map<string, DayAgg>; streams: Map<string, StreamAgg> } {
  ...
  for (const row of rows) {
    ...
    for (const item of items) {
      ...
      const stream = ensureStream(streams, parsed.id, parsed.displayName)
      stream.viewerMinutes += parsed.viewers
      stream.peakViewers = Math.max(stream.peakViewers, parsed.viewers)
...
function readItems(payloadJson: string): RawItem[] {
  try {
...
function parseItem(item: RawItem): { id: string; displayName: string; viewers: number } | null {
  const channel = isRecord(item.channel) ? item.channel : null
  const live = isRecord(item.livestream) ? item.livestream : null
  const slugValue = item.channelLogin ?? item.slug ?? item.username ?? item.user_slug ?? channel?.slug ?? channel?.username ?? channel?.name
  const nameValue = item.displayName ?? item.name ?? item.username ?? channel?.displayName ?? channel?.name ?? channel?.username ?? slugValue
  const viewers = safeNumber(item.viewers ?? item.viewer_count ?? item.viewerCount ?? live?.viewer_count)
  const id = slug(slugValue ?? nameValue ?? '')
  if (!id || viewers <= 0) return null
  return { id, displayName: String(nameValue ?? id), viewers }
}
...
function buildTopStreamers(streams: Map<string, StreamAgg>, previous: Map<string, StreamAgg>) {
  ...
  return ranked.map((stream, index) => {
    ...
    return { streamerId: stream.id, displayName: stream.displayName, viewerMinutes: Math.round(stream.viewerMinutes), peakViewers: Math.round(stream.peakViewers), avgViewers: stream.observedMinutes > 0 ? Math.round(stream.viewerMinutes / stream.observedMinutes) : 0, observedMinutes: stream.observedMinutes, rankByViewerMinutes: index + 1, rankByPeak: peakRank.get(stream.id) ?? index + 1, changePct }
  })
}
...
function buildSummary(daily: PublicDay[], topStreamers: ReturnType<typeof buildTopStreamers>) {
  ...
  return { totalViewerMinutes, peakViewers: peakDay.peakViewers, peakDay: peakDay.day, topStreamer: topStreamers[0] ?? null, biggestRise: null, coverageState }
}
...
function buildCoverage(period: { from: string; to: string }, daily: PublicDay[]) {
  ...
  return { state, observedDays, missingDays, partialDays, notes: [`${observedDays} of ${days} requested days have observed Kick snapshots.`] }
}

function toPublicDay(day: DayAgg): PublicDay {
  return { day: day.day, totalViewerMinutes: Math.round(day.totalViewerMinutes), peakViewers: Math.round(day.peakViewers), peakStreamerName: day.peakStreamerName, observedStreamCount: day.observedStreamCount, observedMinutes: day.observedMinutes, coverageState: day.demoCount > 0 ? 'demo' : day.observedMinutes >= 60 ? 'good' : 'partial' }
}
...
function previousPeriod(from: string, to: string): { from: string; to: string } { const days = dayCount(from, to); const end = new Date(`${from}T00:00:00.000Z`); end.setUTCDate(end.getUTCDate() - 1); const start = new Date(end); start.setUTCDate(end.getUTCDate() - days + 1); return { from: dayString(start), to: dayString(end) } }
function errorResponse(period: { from: string; to: string; label: string }, metric: string, code: string, message: string, status: number) { return Response.json({ source: 'api', state: 'error', platform: 'kick', period: { ...period, days: dayCount(period.from, period.to) }, metric, summary: null, daily: [], topStreamers: [], coverage: { state: 'missing', observedDays: 0, missingDays: 0, partialDays: 0, notes: [message] }, notes: [], error: { code, message } }, { status, headers: { 'cache-control': 'no-store' } }) }
...
function isRecord(value: unknown): value is RawItem { return typeof value === 'object' && value !== null }
function safeNumber(value: unknown): number { if (typeof value === 'number' && Number.isFinite(value)) return Math.max(0, Math.round(value)); if (typeof value === 'string') { const parsed = Number(value.replace(/,/g, '')); return Number.isFinite(parsed) ? Math.max(0, Math.round(parsed)) : 0 } return 0 }
