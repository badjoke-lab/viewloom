export type SnapshotRow = { bucket_minute: string; total_viewers: number; stream_count: number; payload_json: string; source_mode: string }
...
export const PERIOD_BASELINE_MINUTES = 60
export const DAILY_BASELINE_MINUTES = 30

export function ranked(streams: Map<string, StreamAgg>, previous: Map<string, StreamAgg>, limit: number, baselineMinutes: number): RankedStream[] {
  const values = [...streams.values()]
  const byMinutes = [...values].sort((a, b) => b.viewerMinutes - a.viewerMinutes).slice(0, limit)
  const peakRank = new Map([...values].sort((a, b) => b.peakViewers - a.peakViewers).map((stream, index) => [stream.id, index + 1]))
...
export function buildPayload(provider: 'twitch' | 'kick', period: Period, metric: string, readPath: string, built: BuiltHistory, extra: Record<string, unknown> = {}) {
...
export function coverageFor(provider: 'twitch' | 'kick', period: Period, daily: DailySummary[]) {
  ...
  const partial = daily.filter((day) => day.coverageState !== 'good' && day.coverageState !== 'demo').map((day) => day.day)
