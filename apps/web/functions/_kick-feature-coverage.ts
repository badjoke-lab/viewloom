...
export async function enrichKickFeatureResponse(env: Env, response: Response): Promise<Response> {
  ...
  try {
    ...
    const existingCoverage = object(payload.coverage) ?? {}
    const existingNotes = Array.isArray(payload.notes) ? payload.notes.filter((item): item is string => typeof item === 'string') : []
    const notes = existingNotes.includes(coverage.publicNote) ? existingNotes : [...existingNotes, coverage.publicNote]
