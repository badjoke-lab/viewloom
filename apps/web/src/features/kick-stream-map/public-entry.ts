...
function renderReadiness(model: PreviewModel, country: KickCountryPreviewModel): void {
  const ready = model.canRenderCountryGeography && country.contractSafe
  setText('[data-kick-preview-state]', ready ? (country.countryRows.length > 0 ? 'Live' : 'Empty') : 'Unavailable')
...
async function renderCountrySurface(model: KickCountryPreviewModel): Promise<void> {
  ...
  const ready = model.allowGeography && model.contractSafe
  ...
  if (!ready || !mapRoot) {
    ...
    setText('[data-kick-preview-gate-heading]', 'Kick Stream Map unavailable')
    setText('[data-kick-preview-gate-detail]', 'The public Country surface is fail-closed because the current API contract is not ready.')
    return
  }
  ...
  if (model.countryRows.length === 0) {
    ...
    if (results) results.hidden = true
    if (metric) metric.disabled = true
    setText('[data-kick-preview-gate-heading]', 'No reviewed Country rows right now')
    setText('[data-kick-preview-gate-detail]', 'The current Kick observation is valid, but no live streams in this snapshot have an accepted reviewed Country placement.')
    return
  }
...
function renderStreamRows(model: KickCountryPreviewModel): void {
  ...
  for (const row of rows.sort((a, b) => b.viewers - a.viewers || a.displayName.localeCompare(b.displayName))) {
    ...
    detail.textContent = `${row.countryCode} · reviewed Country terminal state`
