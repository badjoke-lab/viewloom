...
export function drawTilesLayer(
  ...
  dpr: number,
  selectedStreamLogin: string | null = null,
): void {
...
function fitText(ctx: CanvasRenderingContext2D, value: string, maxWidth: number): string {
  return fitGraphemes(ctx, segmentGraphemes(normalizeLabel(value)), maxWidth, true)
}
...
function momentumColor(momentum: number): string {
  if (momentum > FLAT_MOMENTUM_THRESHOLD) return 'rgba(167,243,208,0.94)'
  if (momentum < -FLAT_MOMENTUM_THRESHOLD) return 'rgba(254,205,211,0.94)'
