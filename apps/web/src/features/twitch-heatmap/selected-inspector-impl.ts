      ...
      <div class="heatmap-selected-inspector__card">
        <div class="heatmap-selected-inspector__head">
          <div class="heatmap-selected-inspector__identity">
            <div class="heatmap-selected-inspector__eyebrow"><span class="heatmap-selected-inspector__live" id="heatmap-inspector-live">Waiting</span><span>Selected stream</span></div>
            <h2 id="heatmap-inspector-title">No stream selected</h2>
            <div id="heatmap-inspector-login" class="heatmap-selected-inspector__login">—</div>
            <p id="heatmap-inspector-body" class="heatmap-selected-inspector__title">Select a tile to inspect its current observed position.</p>
          </div>
          <div id="heatmap-inspector-snapshot" class="heatmap-selected-inspector__snapshot">Snapshot —</div>
        </div>
        <div class="heatmap-selected-inspector__metrics">
          ${metric('Viewers', 'heatmap-inspector-viewers')}
          ${metric('Observed rank', 'heatmap-inspector-rank')}
          ${metric('Observed share', 'heatmap-inspector-share')}
          ${metric('Momentum', 'heatmap-inspector-momentum', 'heatmap-inspector-momentum-note')}
          ${metric('Activity', 'heatmap-inspector-activity', 'heatmap-inspector-activity-note')}
        </div>
        <div class="heatmap-selected-inspector__observations">
          ${observation('Observed since', 'heatmap-inspector-observed-since')}
          ${observation('Observed duration', 'heatmap-inspector-observed-duration')}
          ${observation('Latest observed peak', 'heatmap-inspector-peak')}
          ${observation('Peak time', 'heatmap-inspector-peak-time')}
        </div>
        <p id="heatmap-inspector-context-note" class="heatmap-selected-inspector__context-note">Recent observation context has not been loaded.</p>
        <div class="heatmap-selected-inspector__actions">
          <a id="heatmap-inspector-link" target="_blank" rel="noreferrer">Open stream</a>
          <a id="heatmap-inspector-battle-lines">Open in Battle Lines</a>
          <a id="heatmap-inspector-history">Review 7-day history</a>
        </div>
      </div>
...
export function renderSelectedInspectorPending(): void {
  ...
  setText('#heatmap-inspector-live', 'Waiting')
  setText('#heatmap-inspector-title', 'No stream selected')
  setText('#heatmap-inspector-login', '—')
  setText('#heatmap-inspector-body', 'Select a tile to inspect its current observed position.')
  setText('#heatmap-inspector-snapshot', 'Snapshot —')
  resetValues()
  setText('#heatmap-inspector-context-note', 'Recent observation context has not been loaded.')
  clearLink('#heatmap-inspector-link', 'Open stream')
  clearLink('#heatmap-inspector-battle-lines', 'Open in Battle Lines')
  clearLink('#heatmap-inspector-history', 'Review 7-day history')
...
export function renderSelectedInspectorUnavailable(title: string, body: string): void {
  requestSerial += 1
  ensureSelectedInspectorShell()
  setText('#heatmap-inspector-live', 'Unavailable')
  setText('#heatmap-inspector-title', title)
  setText('#heatmap-inspector-login', '—')
  setText('#heatmap-inspector-body', body)
  setText('#heatmap-inspector-snapshot', 'Snapshot unavailable')
  resetValues()
  setText('#heatmap-inspector-context-note', 'No selected-stream context is available for this page state.')
  clearLink('#heatmap-inspector-link', 'Open stream')
  clearLink('#heatmap-inspector-battle-lines', 'Open in Battle Lines')
  clearLink('#heatmap-inspector-history', 'Review 7-day history')
}
...
export function renderSelectedStreamInspector(input: SelectedInspectorInput): void {
  ...
  setText('#heatmap-inspector-live', 'Live')
  setText('#heatmap-inspector-title', item.displayName)
  setText('#heatmap-inspector-login', `@${item.channelLogin}`)
  setText('#heatmap-inspector-body', item.title || `${provider.label} stream observed in the latest Heatmap snapshot.`)
  setText('#heatmap-inspector-snapshot', `Snapshot ${formatInstant(latest.collected_at)}`)
  setText('#heatmap-inspector-viewers', item.viewers.toLocaleString())
  setText('#heatmap-inspector-rank', rank ? `#${rank}` : 'Unavailable')
  setText('#heatmap-inspector-share', `${(share * 100).toFixed(2)}%`)
  setText('#heatmap-inspector-momentum', formatSignedPercent(item.momentum))
  setText('#heatmap-inspector-momentum-note', `${direction} · ${bucketMinutes}m window`)
  setText('#heatmap-inspector-activity', activity.value)
  setText('#heatmap-inspector-activity-note', activity.note)
  setText('#heatmap-inspector-observed-since', 'Loading…')
  setText('#heatmap-inspector-observed-duration', 'Loading…')
  setText('#heatmap-inspector-peak', 'Loading…')
  setText('#heatmap-inspector-peak-time', 'Loading…')
  setText('#heatmap-inspector-context-note', 'Reading the current contiguous observation run from stored 5-minute snapshots.')
  setLink('#heatmap-inspector-link', streamUrl, `Open on ${provider.label}`)
  setLink('#heatmap-inspector-battle-lines', links.battleLines, 'Open in Battle Lines')
  setLink('#heatmap-inspector-history', links.history, 'Review 7-day history')
...
function renderContext(context: StreamContext, fallbackBucketMinutes: number): void {
  ...
  setText('#heatmap-inspector-observed-since', formatInstant(context.observedSince))
  setText('#heatmap-inspector-observed-duration', formatObservationDuration(context.observedDurationMinutes ?? null, Boolean(context.windowTruncated), bucketMinutes))
  setText('#heatmap-inspector-peak', typeof context.peakViewers === 'number' ? context.peakViewers.toLocaleString() : 'Unavailable')
  setText('#heatmap-inspector-peak-time', context.peakAt ? formatInstant(context.peakAt) : 'Unavailable')
  const samples = context.sampleCount ?? 0
  const boundary = context.windowTruncated ? ' The run reaches the 24-hour query boundary.' : ''
  setText('#heatmap-inspector-context-note', `Based on ${samples.toLocaleString()} consecutive stored observation sample${samples === 1 ? '' : 's'}.${boundary}`)
}
