...
export function installHeatmapLayoutMode(): () => void {
  ...
  const apply = (next: HeatmapLayoutMode, persist: boolean): void => {
    ...
    document.body.dataset.heatmapLayout = next

    for (const button of buttons) {
      const active = button.dataset.heatmapLayout === next
    ...
    document.querySelectorAll<HTMLElement>('.head-facts .fact').forEach((fact) => {
      ...
      if (value) value.textContent = next === 'wide' ? 'Wide' : 'Split'
    })
    ...
    window.dispatchEvent(new CustomEvent('viewloom:heatmap-layout-change', { detail: { mode: next } }))
  }
