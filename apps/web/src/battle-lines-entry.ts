...
function render(): void {
  ...
  if (summary) summary.innerHTML = `<div><span>${state.mode === 'inspect' ? 'Inspecting' : state.mode === 'custom' ? 'Custom' : 'Recommended'}</span><h2>${pair[0].name} vs ${pair[1].name}</h2><p>${info.trend} gap · ${format(info.gap)}</p></div><strong>Gap ${format(info.gap)}</strong><strong>${info.trend} ${signed(info.delta)}</strong><strong>Latest reversal 00:40</strong><strong>Fastest challenger Jynxzi</strong>`
...
function renderChart(pair: [Line, Line]): void {
  ...
  host.innerHTML = `<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="Battle Lines chart"><rect width="${w}" height="${h}" rx="18" fill="#07101d"/>${ticks(max).map((tick) => `<line x1="${left}" x2="${w - right}" y1="${top + plotH - plotH * tick / max}" y2="${top + plotH - plotH * tick / max}" stroke="rgba(148,163,184,.16)"/><text x="${left - 12}" y="${top + plotH - plotH * tick / max + 4}" text-anchor="end" fill="#9fb0ca" font-size="13">${state.metric === 'indexed' ? Math.round(tick) : compact(tick)}</text>`).join('')}<polygon points="${band}" fill="rgba(96,165,250,.12)"/>${lines.filter((line) => !state.pair.includes(line.id)).map((line) => `<path d="${path(line)}" fill="none" stroke="${line.color}" stroke-width="2" opacity=".28"/>`).join('')}<path d="${path(pair[0])}" fill="none" stroke="${pair[0].color}" stroke-width="4"/><path d="${path(pair[1])}" fill="none" stroke="${pair[1].color}" stroke-width="4"/><line x1="${x(state.selected)}" x2="${x(state.selected)}" y1="${top}" y2="${h - bottom}" stroke="rgba(255,255,255,.84)"/><text x="${x(state.selected)}" y="${top - 12}" text-anchor="middle" fill="#eef4ff">${time(state.selected)}</text></svg>`
...
function renderInspector(pair: [Line, Line], info: { gap: number; delta: number; trend: string; leader: Line }): void {
  ...
  el.innerHTML = `<div><span>Selected time</span><strong>${time(state.selected)}</strong><button data-live type="button">Jump to live</button></div><div><span>Pair</span><strong>${pair[0].name} vs ${pair[1].name}</strong><small>Leader · ${info.leader.name}</small></div><div><span>Gap</span><strong>${format(info.gap)}</strong></div><div><span>Trend</span><strong>${info.trend}</strong><small>${signed(info.delta)}</small></div><div><span>Viewers</span><p>${pair[0].name}<b>${format(a)}</b></p><p>${pair[1].name}<b>${format(b)}</b></p></div><div><span>Change</span><p>${pair[0].name}<b>${signed(a - pair[0].values[prev])}</b></p><p>${pair[1].name}<b>${signed(b - pair[1].values[prev])}</b></p></div>`
...
function renderLists(): void {
  ...
  if (rev) rev.innerHTML = `<h2>Latest Reversals</h2><div><button type="button">00:40 HasanAbi passed TheBurntPeanut</button><button type="button">00:35 HasanAbi passed fauxty</button><button type="button">00:10 TheBurntPeanut passed xQc</button></div>`
  const sec = document.querySelector<HTMLElement>('[data-secondary]')
  if (sec) sec.innerHTML = `<h2>Secondary Battles</h2><div><button type="button" data-pair="burnt:xqc">TheBurntPeanut vs xQc</button><button type="button" data-pair="jynxzi:burnt">Jynxzi vs TheBurntPeanut</button></div>`
  sec?.querySelectorAll<HTMLButtonElement>('[data-pair]').forEach((button) => button.addEventListener('click', () => { const [a, b] = String(button.dataset.pair).split(':') as Pair; state = { ...state, mode: 'custom', pair: [a, b] }; render() }))
...
function getPair(): [Line, Line] | null { const a = state.lines.find((line) => line.id === state.pair[0]); const b = state.lines.find((line) => line.id === state.pair[1]); return a && b ? [a, b] : null }
