...
type Stream = { displayName:string; title:string; category:string; viewers:number; changePct:number|null; direction:string; url:string }
type Battle = { left:{displayName:string}; right:{displayName:string}; gap:number }
type Payload = {
  version:string; platform:Platform; state:string; source:string; updatedAt:string|null;
  coverage:{label:string;note:string};
  now:{observedStreams:number;observedViewers:number;largestStream:Stream|null;topStreams:Stream[];fastestRiser:Stream|null;closestGap:Battle|null;topCategory:{name:string;viewers:number;streams:number}|null};
  today:{observedPeak:number|null;peakTime:string|null;currentObservedViewers:number;topByViewerMinutes:{displayName:string;viewerMinutes:number}|null;closestCurrentBattle:Battle|null};
  recent:{latestCompletedDay:string|null;topStreamer:{displayName:string;viewerMinutes:number}|null;biggestRise:{displayName:string;changePct:number}|null;coverageState:string|null;trend:Array<{day:string;peakViewers:number;coverageState:string}>};
  signals:Array<{label:string;summary:string;observedAt:string|null}>;
  availability:{latestReversal:string}; error?:{message:string}
}
...
async function load(active: Platform): Promise<void> {
  ...
  try {
    ...
    if (payload.version !== 'viewloom-home-v1' || payload.platform !== active) throw new Error('Unexpected Home payload.')
    render(payload)
  } catch (error) {
...
function render(payload: Payload): void {
  ...
  text('home-state', label(payload.state))
  ...
  text('home-strip-source', payload.state === 'demo' ? 'Demo' : 'Real')
...
function headerStatus(payload: Payload): void {
  ...
  dot.className = 'dot'
  node.replaceChildren(dot, document.createTextNode(`${label(payload.state)} · ${payload.updatedAt ? ago(payload.updatedAt) : 'Update unavailable'}`))
}
...
function liveRows(payload: Payload): void {
  for (let index = 0; index < 5; index += 1) {
    ...
    text(`home-live-name-${index}`, stream.displayName)
    text(`home-live-category-${index}`, stream.category || stream.title || 'Category unavailable')
    text(`home-live-viewers-${index}`, number(stream.viewers))
    const movement = stream.changePct == null ? 'Unavailable' : `${stream.changePct >= 0 ? '+' : ''}${(stream.changePct * 100).toFixed(1)}%`
  ...
  const caption = payload.now.topStreams.length ? `Showing ${payload.now.topStreams.length} of ${number(payload.now.observedStreams)} observed streams` : payload.state === 'empty' ? 'No qualifying live streams were observed.' : 'Live ranking is unavailable.'
...
function today(payload: Payload): void {
  ...
  text('home-today-reversal', payload.availability.latestReversal === 'unavailable' ? 'Unavailable in Home payload' : 'No reversal detected')
...
function recent(payload: Payload): void {
  ...
  for (let index = 0; index < 7; index += 1) {
    ...
    if (item) item.title = `${day(point.day)} · ${number(point.peakViewers)} peak viewers · ${label(point.coverageState)}`
  }
...
function fail(message: string): void {
  ...
  text('home-strip-source', 'Real source unavailable')
  text('home-status-note', `Home data could not be loaded. ${message}`)
  text('home-live-caption', 'The real Home payload is unavailable. No demo ranking was substituted.')
}

function text(id: string, value: string): void { const node = document.getElementById(id); if (node) node.textContent = value }
function meter(id: string, value: number, maximum: number): void { const node = document.getElementById(id); if (node) node.style.width = `${Math.round(Math.max(0, Math.min(1, value / maximum)) * 100)}%` }
...
function ago(value: string): string { const ms = Date.now() - Date.parse(value); if (!Number.isFinite(ms)) return 'Update unavailable'; const minutes = Math.max(0, Math.floor(ms / 60000)); if (minutes < 1) return 'Just now'; if (minutes < 60) return `${minutes}m ago`; const hours = Math.floor(minutes / 60); return hours < 24 ? `${hours}h ago` : `${Math.floor(hours / 24)}d ago` }
function time(value: string|null): string { if (!value) return 'Unavailable'; const date = new Date(value); return Number.isNaN(date.getTime()) ? 'Unavailable' : `${date.toISOString().slice(11,16)} UTC` }
function day(value: string): string { const date = new Date(`${value}T00:00:00.000Z`); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'}).format(date) }
function shortDay(value: string): string { const date = new Date(`${value}T00:00:00.000Z`); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',timeZone:'UTC'}).format(date) }
function label(value: string): string { return value.split('_').map((word) => word ? word[0].toUpperCase() + word.slice(1) : word).join(' ') }
