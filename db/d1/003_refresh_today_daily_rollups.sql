-- Refresh today's daily_rollups rows from raw minute_snapshots.
-- Apply after db/d1/001_daily_rollups.sql.
-- Safe to rerun.
...
WITH target AS (
  ...
  WHERE substr(bucket_minute, 1, 10) = (SELECT day FROM target)
  ...
  WHERE substr(m.bucket_minute, 1, 10) = (SELECT day FROM target)
), stream_totals AS (
  SELECT
    provider,
    day,
