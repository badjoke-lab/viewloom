...
function validatePassedProvider(provider, item) {
  ...
  assert.ok(Number.isFinite(measurements.baselineIntervalMs) && measurements.baselineIntervalMs >= 0)
  assert.ok(Number.isFinite(measurements.postIntervalMs) && measurements.postIntervalMs > 0)
  assert.ok(Number.isFinite(measurements.collectorIntervalDeltaMs) && measurements.collectorIntervalDeltaMs >= 0)
  assert.ok(measurements.collectorIntervalDeltaMs <= contract.acceptanceThresholds.collectorIntervalDeltaMsPerProviderMax)
