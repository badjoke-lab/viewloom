...
for (const fragment of [
  ...
  'Identity-resolution searches count toward the same five-attempt identity budget.',
  'increment the per-identity counter before each distinct external search/lookup attempt',
  'when the counter reaches five without another terminal result, stop research and record `no_qualifying_evidence`',
  'a sixth distinct search/lookup attempt is forbidden',
  'reviewBudgetValid=false',
  'must not mutate accepted reviewed evidence',
  '120-minute',
  'without refill',
...
console.log(JSON.stringify({
  ...
  identityResolutionSharesSearchBudget: true,
  sixthSearchForbidden: true,
