...
const files = {
  ...
  packageAcceptance: 'docs/audits/12a5-twitch-replacement-audit-checkpoint-package-acceptance.json',
...
assert.equal(existsSync(files.trigger), false, 'production checkpoint trigger must be absent before the trigger PR')
...
assert.equal(pkg.status, 'accepted')
...
assert.equal(pkg.acceptance.packagePr, 665)
assert.equal(pkg.acceptance.packageCandidateHeadSha, '3e5022f9d3d84cd7b5a75549c0deb69c73e201ca')
assert.equal(pkg.acceptance.packageMergeSha, '317675ea9a6256eb61bf36f8ec9d7a51ffdfff2a')
assert.equal(pkg.acceptance.acceptancePr, 666)
assert.equal(pkg.acceptance.validationRunId, 30476596379)
assert.equal(pkg.acceptance.validationJobId, 90659857133)
...
assert.equal(acceptance.schemaVersion, 'viewloom-12a5-twitch-replacement-audit-checkpoint-package-acceptance-v1')
assert.equal(acceptance.status, 'accepted')
assert.equal(acceptance.acceptancePr, 666)
assert.equal(acceptance.packagePr, 665)
assert.equal(acceptance.packageCandidateHeadSha, pkg.acceptance.packageCandidateHeadSha)
assert.equal(acceptance.packageMergeSha, pkg.acceptance.packageMergeSha)
assert.equal(acceptance.validation.workflowRunId, pkg.acceptance.validationRunId)
assert.equal(acceptance.validation.workflowJobId, pkg.acceptance.validationJobId)
assert.equal(acceptance.validation.conclusion, 'success')
assert.equal(acceptance.validation.triggerAbsentPass, true)
assert.equal(acceptance.validation.productionCheckpointJobSkippedPass, true)
assert.equal(acceptance.acceptedCapabilities.checkpointModeOnly, true)
assert.equal(acceptance.acceptedCapabilities.checkpointNonAuthorizing, true)
assert.equal(Object.values(acceptance.boundaries).every((value) => value === false), true)
...
for (const forbidden of ['INSERT INTO', 'UPDATE ', 'DELETE FROM', 'ALTER TABLE']) assert.equal(runner.includes(forbidden), false)
...
for (const [path, fragments] of Object.entries({
  [files.schedule]: ['Checkpoint package PR #665', 'Checkpoint package acceptance PR #666', 'work-659-twitch-replacement-audit-checkpoint-trigger'],
  [files.roadmap]: ['Checkpoint package PR #665', 'acceptance PR #666', 'work-659-twitch-replacement-audit-checkpoint-trigger'],
  [files.spec]: ['Checkpoint package PR: #665', 'Checkpoint package acceptance PR: #666', 'work-659-twitch-replacement-audit-checkpoint-trigger'],
  [files.wip]: ['Checkpoint package: PR #665', 'Checkpoint package acceptance: PR #666', 'work-659-twitch-replacement-audit-checkpoint-trigger'],
})) {
...
console.log(JSON.stringify({
  ...
  packagePr: pkg.acceptance.packagePr,
  packageAcceptancePr: pkg.acceptance.acceptancePr,
