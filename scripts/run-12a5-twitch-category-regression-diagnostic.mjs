...
async function run() {
  ...
  const outputDir = path.resolve(process.env.OUTPUT_DIR ?? 'artifacts/12a5-twitch-category-regression-diagnostic')
...
WHERE provider = 'twitch' AND collected_at >= '${START_AT}'
  AND bucket_minute >= (
    SELECT datetime(MAX(bucket_minute), '-30 minutes')
    FROM minute_snapshots
    WHERE provider = 'twitch' AND collected_at >= '${START_AT}'
      AND json_extract(payload_json, '$.categoryContractVersion') = 'category-source-v1'
  )
  AND bucket_minute <= (
    SELECT datetime(MAX(bucket_minute), '+60 minutes')
    FROM minute_snapshots
    WHERE provider = 'twitch' AND collected_at >= '${START_AT}'
      AND json_extract(payload_json, '$.categoryContractVersion') = 'category-source-v1'
  )
...
`.trim()
  ...
  const plainBindings = bindings
    ...
    .map((value) => ({ name: String(value.name ?? ''), text: value.name === 'CATEGORY_CAPTURE_ENABLED' ? String(value.text ?? '') : '[redacted-or-irrelevant]' }))

  const evidence = {
    schemaVersion: 'viewloom-12a5-twitch-category-regression-diagnostic-v1',
...
function d1(configPath, sql) {
  ...
  return flatten(parseJson(result.stdout || result.stderr))
}
...
function sanitizeRow(row) { return Object.fromEntries(Object.entries(row).map(([key, value]) => [key, key.includes('error') ? safe(value) : value])) }
