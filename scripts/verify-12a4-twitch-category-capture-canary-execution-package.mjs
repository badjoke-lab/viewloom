...
assert.ok(note.includes('Candidate execution/evidence package only. No trigger exists and production execution is not authorized.'))
...
if (contract.status === 'accepted') {
  assert.ok(Number.isInteger(contract.acceptance.pr))
  assert.ok(typeof contract.acceptance.validatedCandidateHeadSha === 'string')
  assert.ok(Number.isInteger(contract.acceptance.workflowRunId))
  assert.ok(Number.isInteger(contract.acceptance.workflowJobId))
