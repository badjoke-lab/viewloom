...
for (const path of ['docs/work-in-progress/u10d-analysis-coherence.md','apps/web/src/live/battle-lines-loading-guard.ts','scripts/u10d_patch_runtime.py','scripts/u10d_patch_tests.py','scripts/u10d_patch_docs.py','.github/workflows/u10d-bootstrap.yml']) assert.equal(existsSync(join(root,path)),false,`temporary or retired U10D file remains: ${path}`)
...
for (const forbidden of ['fetch(','new MutationObserver','setInterval(','addEventListener(']) assert.equal(dayFlow.includes(forbidden),false,`Day Flow helper owns runtime state: ${forbidden}`)
...
for (const forbidden of ['state.selectedBattleId = payload.primaryBattle.id','state.selectedBattleId = next.primaryBattle?.id ?? null','battle.id === data.primaryBattle?.id && !state.manualBattle','state.manualBattle = state.selectedBattleId !== data.primaryBattle?.id','window.fetch =','window.history.replaceState =','URLSearchParams.prototype.get =','new MutationObserver',"next.set('point'"]) assert.equal(battle.includes(forbidden),false,`stale or intercepted Battle Lines owner remains: ${forbidden}`)
...
for (const source of [battleLayout,battleLink]) for (const forbidden of ['window.fetch =','window.history.replaceState =','URLSearchParams.prototype.get =','new MutationObserver']) assert.equal(source.includes(forbidden),false,`Battle helper contains forbidden architecture: ${forbidden}`)
