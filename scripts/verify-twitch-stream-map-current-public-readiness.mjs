...
const liveResult = JSON.parse(readFileSync('docs/audits/twitch-stream-map-current-review-queue-live-result-2026-09-06.json', 'utf8'))
const acquisitionResult = JSON.parse(readFileSync('docs/audits/twitch-stream-map-current-temporal-evidence-acquisition-result-2026-09-06.json', 'utf8'))
...
const streamMapCoreConsumesStableId = streamMapCoreSource.includes('value.twitchUserId ?? value.user_id') &&
  streamMapCoreSource.includes('stableIdentityStreams') && streamMapCoreSource.includes('missingStableIdentityStreams')
const currentResponseCoreReady = responseCoreSource.includes("stableIdentity: 'twitchUserId'") &&
  responseCoreSource.includes('stableTwitchUserIdRequired: true') && responseCoreSource.includes("reason: 'stable_identity_unavailable'") &&
  responseCoreSource.includes('publicActivationAuthorized: false')
const publicCurrentRouteWired = /normalized\s*===\s*['"]current['"]/.test(publicRouteSource) || /GeographyMode[^\n]*['"]current['"]/.test(publicRouteSource)
...
const readiness = {
  schemaVersion: 'viewloom-twitch-stream-map-current-public-readiness-v0.3', provider: 'twitch', layer: 'current',
  publicCurrentActivationReady: blockers.length === 0, blockers,
  stages: {
  ...
  evidence: {
    sourceProbeRun, sourceObservedAt: acquisitionResult?.sourceProbe?.observedAt ?? null,
    liveSampleSize, liveDuplicateStableIdentityRowsDropped, liveReviewableCandidates, liveConflictingCandidates,
    reviewedCandidates, noFreshQualifyingEvidence, conflictUnmapped, sameCountryGranularityMachineConflictsClosed,
    freshQualifyingEvidence: freshReviewedEvidence, acceptedCurrentPlacement,
  },
  invariant: {
    stableIdentity: 'twitchUserId', loginIsStableIdentity: false, currentUiRemainsDisabled,
    candidateOnlyPlacementAllowed: false, titleOrTagCanPlaceCurrent: false, baseMutationAuthorized: false,
    currentFromBaseInferenceAllowed: false, providerAggregationAllowed: false, preciseAddressPublished: false,
    coordinatesPublished: false, publicActivationAuthorized: false,
  },
}
...
assert.equal(sourceProbeRun, 34036562297, 'Current evidence review must point at the latest September 6 review-queue probe')
...
assert.equal(liveDuplicateStableIdentityRowsDropped, 0)
assert.equal(liveReviewableCandidates, 10)
assert.equal(liveConflictingCandidates, 2)
...
assert.equal(reviewedCandidates, 10)
assert.equal(acquisitionEntries.length, reviewedCandidates)
assert.equal(noFreshQualifyingEvidence, 10)
assert.equal(conflictUnmapped, 0)
assert.equal(sameCountryGranularityMachineConflictsClosed, 2)
assert.equal(acquisitionEntries.filter((entry) => entry?.outcome === 'conflict_unmapped').length, 0)
assert.equal(acquisitionEntries.filter((entry) => entry?.outcome === 'no_fresh_qualifying_temporal_evidence').length, 10)
...
assert.equal(streamMapCoreConsumesStableId, true)
assert.equal(currentResponseCoreReady, true)
...
assert.equal(currentUiRemainsDisabled, true)
...
assert.deepEqual(readiness.blockers, ['production_twitch_snapshot_does_not_retain_user_id','public_current_geography_mode_not_wired','no_fresh_reviewed_current_evidence'])
...
for (const key of ['loginIsStableIdentity','candidateOnlyPlacementAllowed','titleOrTagCanPlaceCurrent','baseMutationAuthorized','currentFromBaseInferenceAllowed','providerAggregationAllowed','preciseAddressPublished','coordinatesPublished','publicActivationAuthorized']) assert.equal(readiness.invariant[key], false)
