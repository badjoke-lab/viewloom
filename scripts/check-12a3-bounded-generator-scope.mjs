...
const implementationCore = new Set([
...
const implementationMode = changed.some((path) => implementationCore.has(path))
...
if (implementationMode) {
  ...
  for (const protectedPath of implementationCore) {
    assert.equal(changed.includes(protectedPath), false, `accepted bounded generator core changed during compatibility work: ${protectedPath}`)
  }
  assert.ok(
    ...
    'bounded generator compatibility mode requires a scope/workflow maintenance change',
  )

  console.log('12A-3 bounded generator scope verification passed.')
  console.log('- mode: accepted compatibility maintenance')
  console.log('- accepted generator SQL, contract, verifier, and fixture remain unchanged')
  console.log('- downstream entry/runtime extensions are verified by collector checks and later-phase gates')
}
