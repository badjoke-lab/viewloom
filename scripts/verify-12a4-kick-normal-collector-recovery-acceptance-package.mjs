...
const tailAudit = read('scripts/run-12a4-kick-normal-collector-tail-audit.mjs')
...
assert.ok(workflow.includes('node scripts/run-12a4-kick-normal-collector-tail-audit.mjs'))
...
assert.ok(workflow.includes('Upload sanitized tail summary only'))
...
assert.ok(tailAudit.includes("spawn('timeout', ["))
assert.ok(tailAudit.includes("'--signal=INT'"))
assert.ok(tailAudit.includes("'--kill-after=10s'"))
assert.ok(tailAudit.includes('const TAIL_DURATION_SECONDS = 420'))
assert.ok(tailAudit.includes("'wrangler@4'"))
assert.ok(tailAudit.includes("'tail'"))
assert.ok(tailAudit.includes("eventType: eventType(event)"))
assert.ok(tailAudit.includes("return 'request_redacted'"))
assert.ok(tailAudit.includes('rawRequestDataStored: false'))
assert.ok(tailAudit.includes('rawLogArtifactStored: false'))
assert.ok(tailAudit.includes("child.kill('SIGKILL')"))
assert.ok(tailAudit.includes('boundedTimeoutExitAccepted'))
assert.ok(tailAudit.includes('safeMessage'))
assert.ok(tailAudit.includes('[redacted-token]'))
assert.equal(tailAudit.includes("'deploy'"), false)
assert.equal(tailAudit.includes("'d1', 'execute'"), false)
assert.equal(tailAudit.includes("'/collect'"), false)
assert.equal(tailAudit.includes('collector-twitch'), false)

for (const source of [runner, runAudit, cloudflareAudit, tailAudit]) {
...
assert.ok(scope.includes('run-12a4-kick-normal-collector-tail-audit.mjs'))
...
console.log(JSON.stringify({
  ...
  boundedSanitizedRuntimeTail: true,
