...
def resolve(config: dict[str, str], now: datetime) -> str:
    ...
    if not requested:
        return "disabled"
    if config.get("provider", "").strip().lower() != "kick":
...
assert resolve({**base, "provider": "twitch"}, start) == "invalid_provider"
...
params = ("kick", "2026-07-15T00:00:00Z", "2026-07-15T00:00:00Z", "category-source-v1", entries)
...
updated_entries = json.dumps([
    {"id": "12", "name": "Just Chatting Updated"},
    {"id": "34", "name": "Games"},
])
before = conn.total_changes
conn.execute(sql, ("kick", "2026-07-15T00:05:00Z", "2026-07-15T00:05:00Z", "category-source-v1", updated_entries))
rename_changes = conn.total_changes - before
assert rename_changes == 1, rename_changes

assert conn.execute("SELECT COUNT(*) FROM provider_category_dictionary WHERE provider = 'twitch'").fetchone()[0] == 0
assert conn.execute("SELECT COUNT(*) FROM provider_category_dictionary WHERE provider = 'kick'").fetchone()[0] == 2
