...
assert.equal(source.providers.twitch.passes.every((item) => item.pairedNonEmpty === item.rowCount), true)
assert.equal(source.providers.twitch.stableFields.every((item) => item.minimumPresenceRatio === 1), true)
assert.equal(source.providers.kick.primaryOfficialLivestreams.stableIdentityNamePairs.every((item) => item.minimumPresenceRatio === 1), true)
...
assert.equal(migration.nextGate.remoteMigrationApplyAuthorized, false)
...
assert.equal(disabled.providers.twitch.checks.categoryPayloadFieldsAbsent, true)
assert.equal(disabled.providers.kick.checks.categoryPayloadFieldsAbsent, true)
