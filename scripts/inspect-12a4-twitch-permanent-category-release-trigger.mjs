...
const CONFIRMATION = 'RUN_TWITCH_PERMANENT_CATEGORY_RELEASE'
...
export function inspectReleaseTrigger({ trigger, contract, gate, eventName = 'pull_request', now = new Date() }) {
  ...
  check('package accepted', gate?.currentWorkstream?.packageAccepted === true, gate?.currentWorkstream?.packageAccepted)
  check('runtime inactive before release', gate?.currentWorkstream?.twitchPermanentCaptureActive === false, gate?.currentWorkstream?.twitchPermanentCaptureActive)
  check('Kick unauthorized', gate?.currentWorkstream?.kickPermanentCaptureAuthorized === false, gate?.currentWorkstream?.kickPermanentCaptureAuthorized)
