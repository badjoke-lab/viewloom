...
async function executeRelease() {
  ...
  const evidence = {
    schemaVersion: 'viewloom-12a4-twitch-permanent-category-release-start-evidence-v1',
    provider: 'twitch',
...
function runObserver({ mode, outputDir, startAt }) {
  fs.mkdirSync(outputDir, { recursive: true })
  const result = spawnSync(process.execPath, ['scripts/run-12a4-twitch-permanent-category-observer.mjs'], {
    cwd: process.cwd(),
    encoding: 'utf8',
    env: {
      ...process.env,
