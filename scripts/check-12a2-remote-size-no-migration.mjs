...
const forbiddenPrefixes = ['apps/web/functions/', 'apps/web/src/', 'workers/', 'db/d1/', 'migrations/']
const forbiddenExact = new Set(['apps/web/wrangler.toml', 'wrangler.toml'])
const forbidden = changed.filter((path) => forbiddenExact.has(path) || forbiddenPrefixes.some((prefix) => path.startsWith(prefix)))
assert.deepEqual(forbidden, [], `12A-2 remote-size gate branch contains migration/runtime/data-path changes:\n${forbidden.join('\n')}`)

console.log('12A-2 remote-size no-migration verification passed.')
