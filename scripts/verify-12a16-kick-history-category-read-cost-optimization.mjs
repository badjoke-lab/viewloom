...
assert.equal((sql.match(/SELECT \? AS provider, \? AS day/g) ?? []).length, 3)
assert.equal((sql.match(/m\.bucket_minute >= \(b\.day \|\| 'T00:00:00\.000Z'\)/g) ?? []).length, 3)
assert.equal((sql.match(/m\.bucket_minute < \(date\(b\.day, '\+1 day'\) \|\| 'T00:00:00\.000Z'\)/g) ?? []).length, 3)
assert.equal(sql.includes('?1'), false)
assert.equal(sql.includes('?2'), false)
...
console.log('12A-16 Kick History category read-cost optimization verified: existing provider/bucket key/index is authoritative, three raw History category paths use two-bind UTC day range predicates without schema/semantic change, the 250k gate is unchanged, and production/permanent runtime remains closed.')
