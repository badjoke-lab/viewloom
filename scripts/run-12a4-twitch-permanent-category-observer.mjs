...
export async function runObserver(options = {}) {
  ...
  try {
    ...
    const tables = runD1Select(normalConfigPath, `SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('minute_snapshots','provider_category_dictionary','streamer_intraday_rollups','intraday_rollup_status') ORDER BY name;`)
...
SELECT COUNT(*) AS category_payload_rows_since_start FROM minute_snapshots WHERE provider = 'twitch' AND collected_at >= '${startAt}' AND json_extract(payload_json, '$.categoryContractVersion') = 'category-source-v1';
SELECT COUNT(*) AS normal_payload_rows_since_start FROM minute_snapshots WHERE provider = 'twitch' AND collected_at >= '${startAt}' AND json_extract(payload_json, '$.categoryContractVersion') IS NULL;
...
function runD1Select(configPath, sql) {
  if (sql.split(';').map((part) => part.trim()).filter(Boolean).some((part) => !/^SELECT\b/i.test(part))) {
