...
try {
  ...
  const forbidden = changed.filter((file) => forbiddenPrefixes.some((prefix) => file.startsWith(prefix)) || forbiddenExact.has(file))
  ...
  console.log(JSON.stringify({ ok: true, canonicalSync: isCanonicalSync, changed, triggerRetired: true, productionFilesChanged: 0 }, null, 2))
} catch (error) {
