...
const need = (path, fragments) => {
  if (!existsSync(join(root, path))) { issues.push(`missing file: ${path}`); return }
...
need('apps/web/scripts/history-ui-h0-browser.mjs', ["schema: 'viewloom-history-ui-h0-baseline-v1'", "phase: 'P9H0'"])
need('docs/product/history-ui-repair-plan.md', ['Status: complete', 'Completed P9H1: PR #434', 'Completed P9H7 production acceptance: PR #451'])
...
if (issues.length) {
  console.error('History UI P9H0 permanent verification did not pass:')
  for (const issue of issues) console.error(`- ${issue}`)
  process.exit(1)
}
console.log('History UI P9H0 permanent verification passed.')
