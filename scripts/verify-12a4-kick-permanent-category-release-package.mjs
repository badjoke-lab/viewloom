...
assert.equal(contract.acceptedPackage.requiredGateSchemaVersion, gate.schemaVersion)
...
assert.equal(contract.acceptance.mergeSha, '7afb81bb9098104107860e9fe6c920c7380964ad')
assert.equal(contract.acceptance.packagePr, 641)
assert.equal(contract.acceptance.workflowRunId, 30089007295)
assert.equal(contract.acceptance.verifyPackageJobId, 89467818501)
assert.equal(contract.acceptance.inspectProductionJobId, 89467818503)
...
requireText(wip, 'Dormant release package PR: #641.', 'active WIP')
...
requireText(schedule, 'Create an exact one-file trigger on main using accepted release merge', 'schedule')
