...
assert.equal(contract.schemaVersion, 'viewloom-kick-stream-map-stable-id-review-queue-contract-v0.1')
assert.equal(contract.provider, 'kick')
assert.equal(contract.mode, 'read_only_preview')
assert.equal(contract.population.livestreamLimit, 100)
...
assert.equal(contract.budgets.livestreamRequestsMax, 1)
...
for (const required of [
  ...
  "new URL('https://api.kick.com/public/v2/livestreams')",
...
assert.equal(worker.includes('country'), false, 'geography fields must not be produced by the identity queue worker')
