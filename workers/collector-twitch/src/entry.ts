...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === TOP20_AUDIT_PATH && request.method === 'POST') {
      if (!TOP20_PREVIEW_HOST.test(url.hostname)) {
        return Response.json({ ok: false, error: 'preview_only' }, { status: 404 })
      }
      try {
        return Response.json({ ok: true, result: await runTop20ExternalYieldSample(env) }, {
...
async function runTop20ExternalYieldSample(env: Env) {
  ...
  return {
    provider: 'twitch',
    mode: 'read_only_fixed_top20_identity_sample',
