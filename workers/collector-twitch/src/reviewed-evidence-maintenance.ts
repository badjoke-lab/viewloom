...
async function captureFixedTop20(env: Env) {
  ...
  const streamsPayload = await streamsResponse.json() as { data?: unknown }
  ...
  const identities = streamsPayload.data.map((raw, index) => {
    const row = raw as TwitchStreamRow
