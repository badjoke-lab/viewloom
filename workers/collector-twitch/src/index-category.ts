...
async function collectTwitch(env: Env) {
  ...
  return {
    ...
    coveredPages,
    hasMore,
    categoryCapture: written.categoryCapture,
    rollupRefresh,
    retentionCleanup,
  }
}
...
async function writeSnapshot(
...
): Promise<{ streamCount: number; totalViewers: number; categoryCapture: CategoryCaptureResult }> {
  ...
  const payload = JSON.stringify(categoryEnabled && encoded
    ? {
        provider: PROVIDER,
  ...
  return { streamCount, totalViewers, categoryCapture }
}
