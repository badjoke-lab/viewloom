...
export type KickCategoryCanaryMode =
  | 'disabled'
  | 'invalid_provider'
  | 'invalid_attempt'
  | 'invalid_window'
...
export type KickCategoryCanaryState = {
  provider: 'kick'
  mode: KickCategoryCanaryMode
  active: boolean
  requested: boolean
  attempt: number | null
...
export function resolveKickCategoryCaptureCanary(
  env: Pick<Env,
  ...
  if (provider !== 'kick') return { ...base, mode: 'invalid_provider', active: false, categoryCaptureEnabled: false }
...
function canaryEnv(env: Env, state: KickCategoryCanaryState): Env {
  return {
    ...env,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    const state = resolveKickCategoryCaptureCanary(env)
  ...
  async scheduled(event: ScheduledEvent, env: Env): Promise<void> {
    const state = resolveKickCategoryCaptureCanary(env)
    console.log(JSON.stringify({ event: 'kick_category_capture_canary_state', ...state }))
