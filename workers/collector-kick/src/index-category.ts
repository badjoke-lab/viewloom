...
type CategoryCaptureResult =
  ...
  | {
      ...
      dictionary: Awaited<ReturnType<typeof writeCategoryDictionary>> | { error: string }
    }
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/insert-fixture' && request.method === 'POST') {
      return out({ ok: true, result: await writeSnapshot(env, fixture, 'fixture', { reason: 'manual_fixture_insert' }) })
    }
...
async function writeSnapshot(env: Env, items: StreamItem[], sourceMode: string, collectorMeta: Raw = {}) {
  ...
  const storedItems = stripCategorySourceFields(items)
  ...
  const payload = categoryEnabled && encoded
    ? { items: storedItems, collectorMeta, ...encoded.payloadFields }
    : { items: storedItems, collectorMeta }
  ...
  await env.DB_KICK_HOT.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind('kick', bucket, now.toISOString(), total, items.length, JSON.stringify(payload), sourceMode).run()

  let categoryCapture: CategoryCaptureResult = { enabled: false }
  if (categoryEnabled && encoded) {
    try {
      const dictionary = await writeCategoryDictionary(
        env.DB_KICK_HOT,
        'kick',
        encoded.dictionaryEntries,
        now.toISOString(),
      )
      categoryCapture = {
        enabled: true,
        observedItems: encoded.observedItems,
        missingItems: encoded.missingItems,
        coverageState: encoded.coverageState,
        dictionary,
      }
    } catch (error) {
      categoryCapture = {
  ...
  return { bucket_minute: bucket, total_viewers: total, stream_count: items.length, source_mode: sourceMode, category_capture: categoryCapture }
}
...
function sanitizeCategoryError(error: unknown): string {
  const message = error instanceof Error ? error.message : String(error)
  return message.replace(/Bearer\s+[A-Za-z0-9._-]+/g, 'Bearer [redacted]').slice(0, 160)
}
